    ...
    fn load_prefix_dd(&mut self) {
        for c in 0..=255 {
            let opcode = match c {
                ...
                0x86 => Some(build_operator_a_idx_offset(
                    Reg16::IX,
                    (operator_add, "ADD"),
                )),
                0x8e => Some(build_operator_a_idx_offset(
                    Reg16::IX,
                    (operator_adc, "ADC"),
                )),
                0x96 => Some(build_operator_a_idx_offset(
                    Reg16::IX,
                    (operator_sub, "SUB"),
                )),
                0x9e => Some(build_operator_a_idx_offset(
                    Reg16::IX,
                    (operator_sbc, "SBC"),
                )),
                0xa6 => Some(build_operator_a_idx_offset(
                    Reg16::IX,
                    (operator_and, "AND"),
                )),
                0xae => Some(build_operator_a_idx_offset(
                    Reg16::IX,
                    (operator_xor, "XOR"),
                )),
                ...
                match p.x {
                    ...
                    2 => {
                        if p.z <= 3 && p.y >= 4 {
                            ...
                            match p.y {
                                0 => Some(build_otim_otdm(true, false)),  // 0x83
                                1 => Some(build_otim_otdm(false, false)), // 0x8b
                                2 => Some(build_otim_otdm(true, true)),   // 0x93
                                3 => Some(build_otim_otdm(false, true)),  // 0x9b
                                _ => Some(build_noni_nop()),
                            }
                        } else if p.z == 4 {
                            match p.y {
                                0 => Some(build_in_block2(true, false)),   // 0x84
                                1 => Some(build_in_block2(false, false)),  // 0x8c
                                2 => Some(build_in_block2(true, true)),    // 0x94
                                3 => Some(build_in_block2(false, true)),   // 0x9c
                                4 => Some(build_out_block2(true, false)),  // 0xa4
                                5 => Some(build_out_block2(false, false)), // 0xac
                                6 => Some(build_out_block2(true, true)),   // 0xb4
                                7 => Some(build_out_block2(false, true)),  // 0xbc
                                _ => Some(build_noni_nop()),
                            }
                        } else {
                    ...
                    3 => match p.z {
                        2 => match p.y {
                            0 => Some(build_inirx_or_indrx(true)),  // 0xc2
                            1 => Some(build_inirx_or_indrx(false)), // 0xca
                            _ => Some(build_noni_nop()), // Invalid instruction NONI + NOP
                        },
                        3 => match p.y {
                        ...
                        7 => match p.y {
                            0 => Some(build_ld_i_hl()),
                            2 => Some(build_ld_hl_i()),
                            _ => Some(build_noni_nop()), // Invalid instruction NONI + NOP
                        },
                        _ => Some(build_noni_nop()), // Invalid instruction NONI + NOP
                    },
